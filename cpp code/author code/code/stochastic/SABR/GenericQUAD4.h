...
double SubIntegral(double sValue, double vValue, double Time, SVProcess& Proc, SquareGrid& GridPts)
{
	...
	double NewvLow = max(Proc.InitvLow(vValue,Time,vDEVIATIONS),vLow);
	double NewvHigh = min(Proc.InitvHigh(vValue,Time,vDEVIATIONS),vHigh);

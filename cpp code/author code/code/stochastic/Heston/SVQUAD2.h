...
double BermCall(double Strike, vector<double> Times, SVProcess& Proc, unsigned long Steps);
...
SquareGrid InitGridForBC(double Strike, double Time, SVProcess& Proc, unsigned long Steps);
...
SquareGrid MakeTheNextGridForBC(double Strike,double Time,double TimeStep,SVProcess& Proc,SquareGrid& OldGrid,unsigned long Steps);
...
double BermCall(double Strike, vector<double> Times, SVProcess& Proc, unsigned long Steps)
{
	...
	SquareGrid BGrid = InitGridForBC(Strike,Time,Proc,Steps);
	...
	while (i > 0)
	{
		...
		BGrid = MakeTheNextGridForBC(Strike,NewTime,TimeStep,Proc,BGrid,Steps);
		i--;
	}

	return SubIntegral(Proc.GetValue(),Proc.GetVar(),Times[0],Proc,BGrid);
}
...
SquareGrid MakeTheNextGridForBC(double Strike,double Time,double TimeStep,SVProcess& Proc,SquareGrid& OldGrid,unsigned long Steps)
{
	...
	for (unsigned long i = 0; i < Steps+1; i++)
	{
		for (unsigned long j = 0; j < Steps+1; j++)
		{
			...
			Result.SetValue(i,j,max(exp(sLow+i*sStep)-Strike,Integral));
		}
	}
